	...
	switch(recvbyte)
	{
		case AP_MSG_LOGIN:
		{
			if(m_state == NO_LOGGED_IN && g_config.getBool(ConfigManager::ADMIN_REQUIRE_LOGIN))
			{
				...
				{
					m_state = LOGGED_IN;
					output->add<char>(AP_MSG_LOGIN_OK);
					addLogLine(LOGTYPE_EVENT, "login ok");
				}
				else
				{
					m_loginTries++;
					output->add<char>(AP_MSG_LOGIN_FAILED);
			...
			if(m_state == ENCRYPTION_NO_SET && Admin::getInstance()->isEncypted())
			{
				...
				switch(keyType)
				{
					case ENCRYPTION_RSA1024XTEA:
					{
						...
						if(!rsa)
						{
							output->add<char>(AP_MSG_ENCRYPTION_FAILED);
						...
						if(RSA_decrypt(rsa, msg))
						{
							...
							output->add<char>(AP_MSG_ENCRYPTION_OK);
			...
			if(m_state == ENCRYPTION_NO_SET && Admin::getInstance()->isEncypted())
			{
				...
				switch(keyType)
				{
					case ENCRYPTION_RSA1024XTEA:
					{
						...
						if(!rsa)
						{
							output->add<char>(AP_MSG_KEY_EXCHANGE_FAILED);
							addLogLine(LOGTYPE_EVENT, "no valid server key type");
							break;
						}

						output->add<char>(AP_MSG_KEY_EXCHANGE_OK);
						output->add<char>(ENCRYPTION_RSA1024XTEA);

						char RSAPublicKey[128];
						rsa->getPublicKey(RSAPublicKey);

						output->add<char>s(RSAPublicKey, 128);
						break;
					}
		...
		case AP_MSG_COMMAND:
		{
			...
			switch(command)
			{
				...
				case CMD_SHALLOW_SAVE_SERVER:
				{
					...
					output->add<char>(AP_MSG_COMMAND_OK);
					break;
				}
				...
				default:
				{
					output->add<char>(AP_MSG_COMMAND_FAILED);
		...
		case AP_MSG_PING:
			output->add<char>(AP_MSG_PING_OK);
			break;

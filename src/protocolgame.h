...
class ProtocolGame : public Protocol
{
	public:
		...
		ProtocolGame(Connection_ptr connection): Protocol(connection)
		{
			...
			m_debugAssertSent = m_acceptPackets = m_spectator = is_spectating = false;
		}
	...
	private:
		...
		void AddTextMessage(OutputMessage_ptr msg, MessageClasses mclass, const std::string& message,
		...
		void MoveUpCreature(OutputMessage_ptr msg, const Creature* creature,
			const Position& newPos, const Position& oldPos, uint32_t oldStackpos);
		void MoveDownCreature(OutputMessage_ptr msg, const Creature* creature,
			const Position& newPos, const Position& oldPos, uint32_t oldStackpos);
		...
		bool m_debugAssertSent, m_acceptPackets, m_spectator, is_spectating;

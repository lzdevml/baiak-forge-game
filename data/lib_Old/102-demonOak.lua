...
positions =
{
	...
	rewardRoom = {x=20, y = 113, z = 8},
	demonOak = {x = 51, y = 127, z = 7}
}
...
questAreaPosition =
{
        {x=45, y=121, z=7, stackpos = 255},
        {x=57, y=131, z=7, stackpos = 255}
}
...
function canEnter(cid, tree)
	if type(questAreaPosition) == "table" and type(questAreaPosition[1]) == "table" and type(questAreaPosition[2]) == "table" and level and type(storages) == "table" then
		...
		local monsters = {}
		local  from = {x=42, y=120, z=7}
		local to = {x=60, y=132, z=7}
...
function checkLib()
	...
	local logFile = "/logs/demonOak.txt"

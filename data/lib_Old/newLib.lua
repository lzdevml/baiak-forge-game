...
function Position(a,b,c) pos = {x=a, y=b, z=c} return pos end
...
function Creature:getStorageValue(id) return getPlayerStorageValue(self.cid, id) end
...
function Creature:setStorageValue(storage, value) return setPlayerStorageValue(self.cid, storage, value) end
...
function Player:getFreeCapacity() return getPlayerFreeCap(self.cid) end
...
function Player:getPosition() return getPlayerPosition(self.cid) end
function Player:getStorageValue(id) return getPlayerStorageValue(self.cid, id) end
function Player:setDirection(dir) return doCreatureSetLookDirection(self.cid, dir) end
...
function Player:removeItem(itemid, count) return doPlayerRemoveItem(self.cid, itemid, count) end
...
function Player:teleportToTown() return doTeleportThing(self.cid, getTownTemplePosition(getPlayerTown(self.cid))) end
function Player:say(text, type, ghost, cid, pos) doCreatureSay(self.cid, text, type or SPEAK_SAY, ghost or false, cid or 0, pos) end
...
function Game.getSpectators(centerPos, rangex, rangey, multifloor) return getSpectators(centerPos, rangex, rangey, multifloor or false) end

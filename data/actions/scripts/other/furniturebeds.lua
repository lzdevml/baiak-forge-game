local beds = {
	...
	[20252] = {{13027, 13028}, {13029, 13030}} -- canopy kit
}
...
function onUse(player, item, fromPosition, target, toPosition, isHotkey)
	...
	for kit, bed in pairs(beds) do
		if bed[1][1] == target.itemid or isInArray({1758, 5502}, target.itemid) then
			toPosition:sendMagicEffect(CONST_ME_POFF)
			toPosition.y = toPosition.y + 1
			internalBedTransform(item, target, toPosition, newBed[1])
			break
		elseif bed[2][1] == target.itemid or isInArray({1756, 5500}, target.itemid) then
			toPosition:sendMagicEffect(CONST_ME_POFF)

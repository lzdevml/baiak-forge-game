...
if (getPlayerVocation(cid) == 4 or getPlayerVocation(cid) == 8) then
	if getPlayerFreeCap(cid) > 2.50 and getPlayerMoney(cid) >= 40000 then
		if doPlayerRemoveMoney(cid, 40000) then
			doPlayerAddItem(cid, 13453, 100)
			doPlayerSendTextMessage(cid, MESSAGE_STATUS_DEFAULT, "Você comprou 100 recargas de exercise club por 40k.")
		...
		doPlayerSendTextMessage(cid, MESSAGE_STATUS_DEFAULT, "Você precisa ter mais que '2.50 oz' e '40k' para comprar 100 exercise club.")
	end
else

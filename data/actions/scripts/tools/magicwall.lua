...
if (getPlayerVocation(cid) >= 0 or getPlayerVocation(cid) <= 8) then
	if getPlayerFreeCap(cid) > 1418.00 and getPlayerMoney(cid) >= 173000 then
		if doPlayerRemoveMoney(cid, 173000) then
			local purple_bp = doPlayerAddItem(cid, 21125, 1)
			for i = 1, tonumber(getItemInfo(21125).maxItems) do
				doAddContainerItem(purple_bp, 2293, 100 or 1)
			end
			doPlayerSendTextMessage(cid, MESSAGE_STATUS_DEFAULT, "Você comprou 1 BP de mw runes por 173k.")
	...
	elseif getPlayerFreeCap(cid) > 70.00 and getPlayerMoney(cid) >= 10800 then
		if doPlayerRemoveMoney(cid, 10800) then
			...
			doPlayerSendTextMessage(cid, MESSAGE_STATUS_DEFAULT, "Você comprou 100 de mw runes por 10.8k.")
			doSendAnimatedText(getPlayerPosition(cid), "$$$$", COLOR_GREY)
		else
			doPlayerSendTextMessage(cid, MESSAGE_STATUS_DEFAULT, "Você precisa ter 10.8k em sua BP para fazer esta compra.")
		end
	else
		doPlayerSendTextMessage(cid, MESSAGE_STATUS_DEFAULT, "Você precisa ter mais que '1418.00 oz' e '173k' para comprar mw ou '70.00 oz' e '10.8k' para comprar 100.")
	end
else

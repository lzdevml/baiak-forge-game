...
function onUse(cid, item, fromPosition, itemEx, toPosition)
	...
	if item.aid == 47035 and getPlayerStorageValue(cid, Storage.FerumbrasAscension.storageTimeZamulosh2) <= os.time() and getPlayerStorageValue(cid, 320987) <= os.time() then
		...
		local config = {
			[1] = {pos = {x = 33680, y = 32741, z = 11}},
			[2] = {pos = {x = 33680, y = 32742, z = 11}},
			[3] = {pos = {x = 33680, y = 32743, z = 11}},
			[4] = {pos = {x = 33680, y = 32744, z = 11}},
			[5] = {pos = {x = 33680, y = 32745, z = 11}}
		}

		local storageTime = Storage.FerumbrasAscension.storageTimeZamulosh
		...
		if playerChecked then
			for i, v in pairs(config) do
				if isPlayer(getTopCreature(config[i].pos).uid) then
					...
					doTeleportThing(player, {x = 33643, y = 32762, z = 11})
					doSendMagicEffect(getCreaturePosition(player), 10)
					if getPlayerStorageValue(player, 1098016) == 4 then
						setPlayerStorageValue(player, 1098016, 5)
					end
			...
			if teleportToRoom then
				stopEvent(ResetArea)
				verificaBoss()
				doSendMagicEffect({x = 33644, y = 32756, z = 11}, 10)
				doCreateMonster("Zamulosh", {x = 33644, y = 32756, z = 11})
			...
			doSendMagicEffect(toPosition,2)
			setPlayerStorageValue(cid, Storage.FerumbrasAscension.storageTimeZamulosh2 + os.time() + 15)
		end

local posInicio = {x = 33410, y = 32453, z = 14}
local posFim = {x = 33440, y = 32482, z = 14}
...
function onUse(cid, item, fromPosition, itemEx, toPosition)
	...
	if item.aid == 47040 and getPlayerStorageValue(cid, Storage.FerumbrasAscension.storageTimeRazzagorn2) <= os.time() and getPlayerStorageValue(cid, 320987) <= os.time() then
		...
		local config = {
			[1] = {pos = {x = 33388, y = 32457, z = 14}},
			[2] = {pos = {x = 33389, y = 32457, z = 14}},
			[3] = {pos = {x = 33390, y = 32457, z = 14}},
			[4] = {pos = {x = 33391, y = 32457, z = 14}},
			[5] = {pos = {x = 33392, y = 32457, z = 14}}
		}

		local storageTime = Storage.FerumbrasAscension.storageTimeRazzagorn
		...
		if playerChecked then
			for i, v in pairs(config) do
				if isPlayer(getTopCreature(config[i].pos).uid) then
					...
					doTeleportThing(player, {x = 33415, y = 32471, z = 14})
					doSendMagicEffect(getCreaturePosition(player), 10)
					if getPlayerStorageValue(player, 1098018) == 4 then
						setPlayerStorageValue(player, 1098018, 5)
					end
			...
			if teleportToRoom then
				stopEvent(ResetArea)
				verificaBoss()
				doSendMagicEffect({x = 33424, y = 32467, z = 14}, 10)
				doCreateMonster("Razzagorn", {x = 33424, y = 32467, z = 14})
			...
			doSendMagicEffect(toPosition,2)
			setPlayerStorageValue(cid, Storage.FerumbrasAscension.storageTimeRazzagorn2 + os.time() + 15)
		end

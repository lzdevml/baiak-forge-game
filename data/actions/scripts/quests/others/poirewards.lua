...
function onUse(cid, item, frompos, item2, topos)
	...
	-- Adicione uma montaria com base no sexo do jogador
                if getPlayerSex(cid) == 1 then
                    doPlayerAddMount(cid, 6) -- Blazing Unicorn
	...
	elseif item.uid == 10550 then
		if getPlayerStorageValue(cid,10550) == -1 then
			...
			setPlayerStorageValue(cid,60455,2) -- quest log
		else
			doPlayerSendTextMessage(cid,MESSAGE_INFO_DESCR,"The chest is empty.")
		end
	...
	elseif item.uid == 60455 then
		if getPlayerStorageValue(cid,60455) == -1 then
			...
			setPlayerStorageValue(cid,60455,1)

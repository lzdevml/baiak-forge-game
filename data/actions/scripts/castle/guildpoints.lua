...
Query = db.getResult("SELECT `last_execute` FROM `guilds` WHERE id = '".. GuildID .."'")
if(Query:getID() == -1) then
return true
end
if Query:getDataInt("last_execute") < os.time() then
local GuildMembers = {}
...
db.executeQuery("UPDATE `guilds` SET `last_execute` = ".. os.time() + (GuildPointsConfigs.ExecuteIntervalHours * 3600) .." WHERE `guilds`.`id` = ".. GuildID ..";")
doSummonCreature("King", {x=32368, y=32230, z=7})
doBroadcastMessage(" O lider da guild \""..guildname.."\" acabou de invocar Yggdrasill no castelo.", 21)
doPlayerSendTextMessage(cid, MESSAGE_STATUS_CONSOLE_ORANGE, "".. #ValidAccounts .." Players received hellcoins")
...
doPlayerSendTextMessage(pid, MESSAGE_INFO_DESCR, "You received 1 hellcoins.")
doPlayerAddItem(pid,2160,1)
break
end
end
end

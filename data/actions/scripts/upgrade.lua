...
local controle = {
    ...
    {level = 1, quantOre = 1, chance = 50},
    {level = 2, quantOre = 1, chance = 20},
    {level = 3, quantOre = 1, chance = 8},
    {level = 4, quantOre = 1, chance = 3}
}
    ...
    if getItemInfo(itemEx.itemid).attack > 0 or getItemInfo(itemEx.itemid).defense > 0 or getItemInfo(itemEx.itemid).armor > 0 then
        for _, upgrade in pairs(controle) do
            ...
            if getItemLevel(itemEx.uid) == upgrade.level then
               if doPlayerRemoveItem(cid, 8306, upgrade.quantOre) then
                  ...
                  if chance <= upgrade.chance then
                      ...
                      --doBroadcastMessage("The player " .. getCreatureName(cid) .. " was successful in upgrading "..getItemNameById(itemEx.itemid).." to level +" ..getItemLevel(itemEx.uid).." Congratulations.", MESSAGE_STATUS_CONSOLE_BLUE)

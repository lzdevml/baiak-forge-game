...
function onUse(cid, item, fromPosition, itemEx, toPosition)
	...
	if((not(isSorcerer(itemEx.uid) or isDruid(itemEx.uid)) or getPlayerLevel(itemEx.uid) < 80) and getPlayerCustomFlagValue(itemEx.uid, PlayerCustomFlag_GamemasterPrivileges) == FALSE) then
		doCreatureSay(itemEx.uid, "Somente feiticeiros e druidas de nível 80 ou acima podem beber esse fluido.", TALKTYPE_WHITE_1)
		return TRUE
	end
	...
	doCreatureSay(itemEx.uid, "Aaaah meu cu...", TALKTYPE_WHITE_1)

...
local function placeMoney(amount, table_middle_pos)
        local remain = amount
        local chaos_essences = 0
 
        if (math.floor(amount / 1) >= 1) then
                chaos_essences = math.floor(amount / 1)
                remain = remain - chaos_essences * 1
        end
        addEvent(doCreateItem, 600, 8981, chaos_essences, table_middle_pos)
end
...
local function rollDice(roll, mp_count, table_left_pos, table_middle_pos, npc)
        ...
        if ((total_g) <= 60 and (total_g) >= 3) then
                ...
                addEvent(doCreateItem, 100, 8981, mp_count, table_middle_pos)
...
function creatureSayCallback(cid, type, msg)
        ...
        local table_left_pos = {x=158, y=54, z=5} -- Pos da frente do Npc onde gira o dado
        local table_middle_pos = {x=158, y=55, z=5} -- Pos do meio onde vai o dinheiro
 
        -- Search for coins on the left and middle tables and create item userdata instances
        local table_middle_mp = getTileItemById(table_middle_pos, 8981)
 
        -- Other variables
        local mp_count = 0
        local ROLL, LOW, HIGH = 0, 1, 2
        posplayer = {x=159, y=56, z=5} -- Pos onde o player precisa estar
        ...
        if isInArray({"H", "HIGH", "high", "h"}, msg) then
                ...
                if (table_middle_mp.uid ~= 0) then
                        ...
                        addEvent(delayMoneyRemoval, 300, 8981, table_left_pos)
                end

...
if guid == nil or player == nil or cost == nil then 
	...
	doPlayerSendTextMessage(cid, MESSAGE_STATUS_CONSOLE_BLUE, "[Hunted System] Hunt has been added successfuly!")
	doBroadcastMessage("[Hunted System]: The player  "..huntN[cid].."  are being hunted., reward "..huntP[cid].."K head has been submitted by "..getPlayerName(cid)..".", MESSAGE_STATUS_CONSOLE_RED)
	return TRUE
end
...
if(not(npcHandler:isFocused(cid))) then
	...
	if talkState[cid] == nil then
        ...
        if (msgcontains(msg, 'hunt') or msgcontains(msg, 'yes') or msgcontains(msg, 'hunting')) and talkState[cid] == 0 then
			selfSay('Oh, you want to kill somebody, {what} {is} {his} {name?}',cid)
			talkState[cid] = 1
		elseif talkState[cid]==1 then
			talkState[cid] = 0
			if checkName(msg) == TRUE then
				huntN[cid] = msg
				selfSay('Okay. How much {GP} the death?  {Example:} {30} {=} {30k}',cid)
				talkState[cid] = 2
			else

...
function onThink()
local npc = getNpcCid()
if (getCreatureOutfit(npc).lookType == 1681) then
      doCreatureChangeOutfit(npc, {lookType = 1680, lookHead = 91, lookBody = 91, lookLegs = 91, lookFeet = 91, lookTypeEx = 91, lookAddons = 3})
else 
      doCreatureChangeOutfit(npc, {lookType = 1681, lookHead = 114, lookBody = 114, lookLegs = 114, lookFeet = 114, lookTypeEx = 114, lookAddons = 3})  
end
npcHandler:onThink()
end
...
local function placeMoney(amount, table_middle_pos)
        ...
        if (math.floor(amount / 1) >= 1) then
                mary_points = math.floor(amount / 1)
                remain = remain - mary_points * 1
        end
        addEvent(doCreateItem, 600, 5953, mary_points, table_middle_pos)
end
...
local function rollDice(roll, mp_count, table_left_pos, table_middle_pos, npc)
        ...
        if ((total_g) <= 60 and (total_g) >= 3) then
                ...
                addEvent(doCreateItem, 100, 5953, mp_count, table_middle_pos)
                doCreatureSay(npc, "´Mínimo 3 Styller-OTs points, Máximo 60.", TALKTYPE_SAY, false, 0)
        end
        return true
end
...
function creatureSayCallback(cid, type, msg)
        ...
        local table_left_pos = {x = 156, y = 41, z = 6} -- Pos da frente do Npc onde gira o dado
        local table_middle_pos = {x = 155, y = 41, z = 6} -- Pos do meio onde vai o dinheiro
 
        -- Search for coins on the left and middle tables and create item userdata instances
        local table_middle_mp = getTileItemById(table_middle_pos, 5953)
 
        -- Other variables
        local mp_count = 0
        local ROLL, LOW, HIGH = 0, 1, 2
        posplayer = {x = 154, y = 42, z = 6} -- Pos onde o player precisa estar
        ...
        if isInArray({"H", "HIGH", "high", "h"}, msg) then
                ...
                if (table_middle_mp.uid ~= 0) then
                        ...
                        addEvent(delayMoneyRemoval, 300, 5953, table_left_pos)
                end

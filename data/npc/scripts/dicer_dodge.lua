...
local function placeMoney(amount, table_middle_pos)
        ...
        addEvent(doCreateItem, 600, 13394, anubis_flasks, table_middle_pos)
end
...
local function rollDice(roll, mp_count, table_left_pos, table_middle_pos, npc)
        ...
        if ((total_g) <= 60 and (total_g) >= 3) then
                ...
                addEvent(doCreateItem, 100, 13394, mp_count, table_middle_pos)
...
function creatureSayCallback(cid, type, msg)
        ...
        local table_left_pos = {x = 139, y = 41, z = 5} -- Pos da frente do Npc onde gira o dado
        local table_middle_pos = {x = 139, y = 42, z = 5} -- Pos do meio onde vai o dinheiro
 
        -- Search for coins on the left and middle tables and create item userdata instances
        local table_middle_mp = getTileItemById(table_middle_pos, 13394)
 
        -- Other variables
        local mp_count = 0
        local ROLL, LOW, HIGH = 0, 1, 2
        posplayer = {x = 140, y = 43, z = 5} -- Pos onde o player precisa estar
        ...
        if isInArray({"H", "HIGH", "high", "h"}, msg) then
                ...
                if (table_middle_mp.uid ~= 0) then
                        ...
                        addEvent(delayMoneyRemoval, 300, 13394, table_left_pos)
                end

...
function onTime()
	...
	for _,o in pairs(config.m) do
	    ...
	    local mensage1 = "[Evento Dota] Liberado! A equipe que derrubar os 3 Geradores primeiro vence. A pedra reaparece em 10 minutos. O evento entrega muita exp e muitos prêmios." -- Mensagem Inicial
		gatepos = {x = 557, y = 420, z = 9, stackpos=1} -- Posição da Pedra
		getgate = getThingfromPos(gatepos)
		if getgate.itemid == 1560 then
...
function startDota()
	local mensage1 = "[Evento Dota] Pedras removidas. Entre na sala Dailly no templo. Ganha o time que destruir os 3 Geradores primeiro. Boa sorte!" -- Mensagem Inicial
	...
	if getgate.itemid == 1560 and getgate2.itemid == 1560 then
		broadcastMessage(mensage1, MESSAGE_EVENT_ADVANCE)
		...
		doCreateItem(1560, {x = 557, y = 420, z = 9})
	end
end
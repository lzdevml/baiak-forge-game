    ...
    if table.find(config.days, os.date("%A-%H:%M")) then
        ...
        if online then
            ...
            if(random_item == 2160) then
                ...
                doBroadcastMessage("[LOTTERY SYSTEM] Ganhador: " .. getCreatureName(winner) .. ", Recebeu: 1 " .. config.crystal_counts .." " .. getItemNameById(random_item) .. "s! Nice! (Proxima lottery Sera daqui a " .. config.lottery_hour .. ")")
            else
                doPlayerSetStorageValue(winner, 281821, os.time() + 3600 * 24)
                doBroadcastMessage("[LOTTERY SYSTEM] Ganhador: " .. getCreatureName(winner) .. ", Recebeu: 1 " ..getItemNameById(random_item) .. "! Nice! (Proxima lottery Sera daqui a " .. config.lottery_hour .. ")")

...
function onPrepareDeath(cid, deathList, lastHitKiller, mostDamageKiller)
	if getPlayerStorageValue(cid, 51) == 1 then
		...
		doPlayerSave(cid)
			...
			doPlayerAddItem(cid,8981,5)
			doPlayerAddItem(cid,6527,50)
			doBroadcastMessage("".. getGlobalStorageValue(5) .." Players do Black Assassins team sobreviveram ao evento e obteram a recompensa de 10 Event Coins e 10 Real Coins!")
	...
	elseif getPlayerStorageValue(cid, 52) == 1 then
		...
		doPlayerSave(cid)
			...
			setGlobalStorageValue(5, getGlobalStorageValue(5)-1)
			doRemoveCondition(cid, CONDITION_OUTFIT)
			doPlayerSendTextMessage(cid, MESSAGE_INFO_DESCR, "[DESERT WAR] Você foi o último sobrevivente do time perdedor e também merece recompensa!")
			doPlayerAddItem(cid,2160,10)
			doPlayerAddItem(cid,6527,10)
			doBroadcastMessage("".. getGlobalStorageValue(4).." Players do Red Barbarians team sobreviveram ao evento e obteram a recompensa de 10 Event Coins e 10 Event Coins!")
			setGlobalStorageValue(4, getGlobalStorageValue(4)-1)
		doPlayerSave(cid)
			for _, cid in ipairs(getPlayersOnline()) do

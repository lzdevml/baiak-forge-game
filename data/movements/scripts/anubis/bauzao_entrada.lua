...
if (not isPlayer(cid)) then
    ...
    if isPlayer(cid) then
		if getPlayerLevel(cid) >= 150 then
			if getPlayerStorageValue(cid, 42025) - os.time() < 0 then
				return false
			else
				doPlayerSendTextMessage(cid, 27, "Você já coletou sua recompensa do Super Baú! Aguarde ".. convertTime(getPlayerStorageValue(cid, 42025) - os.time()) .." abrir novamente.")
				doTeleportThing(cid, fromPosition, true)
				doSendMagicEffect(getThingPos(cid), CONST_ME_MAGIC_BLUE)
			end
		else
			doPlayerSendTextMessage(cid, 27, "Você precisa ser level 150 ou superior acessar o Super Baú!")
			doTeleportThing(cid, fromPosition, true)
			doSendMagicEffect(getThingPos(cid), CONST_ME_MAGIC_BLUE)
		end
	end

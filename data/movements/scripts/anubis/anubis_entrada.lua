    ...
    if isPlayer(cid) then
		if getPlayerLevel(cid) >= 200 then
			if getPlayerStorageValue(cid, 42014) - os.time() < 0 then
				...
				doPlayerSendTextMessage(cid, 27, "Seu tempo para Master Daily ainda não expirou! Aguarde ".. convertTime(getPlayerStorageValue(cid, 42014) - os.time()) .." para acessar essa área.")
				doTeleportThing(cid, fromPosition, true)
				doSendMagicEffect(getThingPos(cid), CONST_ME_MAGIC_BLUE)
			end
		else
			doPlayerSendTextMessage(cid, 27, "Você precisa ser level 200 ou superior para fazer a Daily Quest!")
			doTeleportThing(cid, fromPosition, true)
			doSendMagicEffect(getThingPos(cid), CONST_ME_MAGIC_BLUE)
		end
	end

    ...
    if isPlayer(cid) then
		if getPlayerLevel(cid) >= 100 then
			if getPlayerStorageValue(cid, 42011) - os.time() < 0 then
				...
				doPlayerSendTextMessage(cid, 27, "Seu tempo para Daily ainda não expirou! Aguarde ".. convertTime(getPlayerStorageValue(cid, 42011) - os.time()) .." para acessar essa área.")

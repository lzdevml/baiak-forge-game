...
function realTeamBattle.announce(times)
	...
	if getGlobalStorageValue(realTeamBattle.countStorage) > 0 then
		...
		addEvent(realTeamBattle.announce,60000,times - 1)
	else
...
function realTeamBattle.winEvent(team,ammount)
	...
	doBroadcastMessage("[Team Battle] "..ammount.." jogadores do time "..teams[team].." venceram o evento e ganharam 15 Event Coins e 150k!")
end
...
function realTeamBattle.empate(team1,team2)
	...
	for _, pid in pairs(realTeamBattle.getPlayers()) do
		...
		if sto == team1 or sto == team2 then
			doPlayerAddItem(pid, 6527, 15)
			doPlayerSendTextMessage(pid,25,"[Team Battle] Parabéns por vencer o evento, você ganhou 15 Event Coins e 150k!")
		end
		realTeamBattle.cleanPlayer(pid)
		doTeleportThing(pid,realTeamBattle.exit)
	end
	...
	setGlobalStorageValue(realTeamBattle.checkStorage,-1)
	realTeamBattle.createStones()
	doBroadcastMessage("[Team Battle] O evento empatou ficando apenas um jogador do time "..teams[team1].." e um do time "..teams[team2]..". Cada um deles ganhou 15 Event Coins e 150k!")
end

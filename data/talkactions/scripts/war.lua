...
if(not enemy) then
  doPlayerSendChannelMessage(cid, "", "Guild \"" .. t[2] .. "\" nao existe.", TALKTYPE_CHANNEL_W, 0)
  return true
end
...
if(isInArray({"accept", "reject", "cancel"}, t[1])) then
  ...
  if(tmp:getID() == -1) then
   doPlayerSendChannelMessage(cid, "", "Atualmente não há convite pendente para uma guerra com " .. enemyName .. ".", TALKTYPE_CHANNEL_W, 0)
   return true
  end
  ...
  if(t[1] == "accept") then
   ...
   _tmp:free()
   if(state) then
    doPlayerSendChannelMessage(cid, "", "Seu saldo de guild é baixo demais para aceitar este convite.", TALKTYPE_CHANNEL_W, 0)
    return true
   end
  ...
  local msg = "aceitaram " .. enemyName .. " convite para guerra."
  if(t[1] == "reject") then
   query = query .. "`end` = " .. os.time() .. ", `status` = 2"
   msg = "rejected " .. enemyName .. " convite para guerra."
  elseif(t[1] == "cancel") then
   query = query .. "`end` = " .. os.time() .. ", `status` = 3"
   msg = "convite cancelado para uma guerra com " .. enemyName .. "."
  else
...
if(t[1] == "invite") then
                ...
                if(tmp:getID() ~= -1) then
                        if(tmp:getDataInt("status") == 0) then
                                if(tmp:getDataInt("guild_id") == guild) then
                                        str = "Você já convidou " .. enemyName .. " para a guerra."
                                else
                                        str = enemyName .. " já te convidei para a guerra."
                                end
   else
    str = "Você já está em guerra com " .. enemyName .. "."
   end
  ...
  if(payment ~= nil) then
   ...
   tmp:free()
   if(state) then
    doPlayerSendChannelMessage(cid, "", "Seu saldo de guild é muito baixo para tal pagamento.", TALKTYPE_CHANNEL_W, 0)
    return true
   end
  ...
  doBroadcastMessage(getPlayerGuildName(cid) .. " Foi convidado " .. enemyName .. " a guerra até " .. frags .. " frags.", MESSAGE_EVENT_ADVANCE)
  return true
end
...
if(tmp:getID() ~= -1) then
  ...
  db.query(query)
  doBroadcastMessage(getPlayerGuildName(cid) .. " tem " .. (status == 4 and "mend fences" or "acabou uma guerra") .. " com " .. enemyName .. ".", MESSAGE_EVENT_ADVANCE)
  return true
end

if(status == 4) then
  doPlayerSendChannelMessage(cid, "", "Atualmente não há trégua de guerra pendente de " .. enemyName .. ".", TALKTYPE_CHANNEL_W, 0)
  return true
end

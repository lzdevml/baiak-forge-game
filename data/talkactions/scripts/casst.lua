function onSay(cid, words, param, channelId)	
        ...
        if(isInArray({'off', 'no', 'disable'}, t[1])) then
                ...
                doPlayerSendTextMessage(cid, MESSAGE_STATUS_CONSOLE_BLUE, "You have disabled your cast.")
        elseif(isInArray({'on', 'yes', 'enable'}, t[1])) then
                data.broadcast = true
                doPlayerSetSpectators(cid, data)
 
                db.executeQuery("UPDATE `players` SET `broadcasting` = 1 WHERE `id` = " .. getPlayerGUID(cid))
                doPlayerSendTextMessage(cid, MESSAGE_STATUS_CONSOLE_BLUE, "You have enabled your cast.")
        elseif(isInArray({'show', 'count', 'see'}, t[1])) then
                if(data.broadcast) then
                        ...
                        if(count > 0) then
                                ...
                                doPlayerSendTextMessage(cid, MESSAGE_STATUS_CONSOLE_BLUE, "None is watching your cast right now.")
                        end
                else
                        doPlayerSendTextMessage(cid, MESSAGE_STATUS_CONSOLE_BLUE, "You are not cast right now.")
                end
        ...
        elseif(isInArray({'password', 'guard'}, t[1])) then
                if(t[2]) then
                        if(isInArray({'off', 'no', 'disable'}, t[2])) then
                                ...
                                data.password = ""
                                doPlayerSetSpectators(cid, data)
                                doPlayerSendTextMessage(cid, MESSAGE_STATUS_CONSOLE_BLUE, "You have removed password for your cast.")
                        else
                                if(data.password:len() ~= 0) then
                                        db.executeQuery("UPDATE `players` SET `broadcasting` = `broadcasting` + 2 WHERE `id` = " .. getPlayerGUID(cid))
                                end
                                ...
                                doPlayerSetSpectators(cid, data)
                                doPlayerSendTextMessage(cid, MESSAGE_STATUS_CONSOLE_BLUE, "You have set new password for your cast.")
                        end
                elseif(data.password ~= "") then
                        ...
                        doPlayerSendTextMessage(cid, MESSAGE_STATUS_CONSOLE_BLUE, "Your cast is currently not protected.")
                end
        elseif(isInArray({'status', 'info'}, t[1])) then
                doPlayerSendTextMessage(cid, MESSAGE_STATUS_CONSOLE_BLUE, "Your cast is currently " .. (data.broadcast and "enabled" or "disabled") .. ".")
        else

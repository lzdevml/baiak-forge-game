...
function onSay(cid, words, param, channel)
	if getPlayerVocation(cid) == 3 or getPlayerVocation(cid) == 7 then -- paly
		...
		if getPlayerStorageValue(cid, storPaly_changeElement) ~= 1 then
			if (getPlayerLevel(cid) >= 8) and (getPlayerMoney(cid) >= 300000) then
				if (getPlayerLevel(cid) >= 8) and (doPlayerRemoveMoney(cid, 300000)) then
					setPlayerStorageValue(cid, storPaly_changeElement, 1)
					doPlayerSendTextMessage(cid, MESSAGE_INFO_DESCR, "Seu novo elemento: Physical")
				else
				...
				doPlayerSendTextMessage(cid, MESSAGE_INFO_DESCR, "Você não tem dinheiro suficiente. Você precisa de 300k para trocar o elemento da arma.")
			end
		else

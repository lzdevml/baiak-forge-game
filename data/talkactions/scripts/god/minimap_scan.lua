...
local function teleportToClosestPosition(player, x, y, z)
   ...
   if not tile or not getTileThingByPos(tile) or --[[tile:hasFlag(TILESTATE_TELEPORT) or]] not player:teleportTo(tile) then
      for distance = 1, 3 do
         -- try to find some close tile
         for changeX = -distance, distance, distance do
            for changeY = -distance, distance, distance do
               tile = {x + changeX, y + changeY, z}
               if tile and getTileThingByPos(tile) and --[[not tile:hasFlag(TILESTATE_TELEPORT) and]] player:teleportTo(tile) then
                  return true
               end
            end
         end
...
function onSay(cid, words, param)
   ...
   local positions = doCutText(param,",")

local arena_storage = 444523
local players_storage = 446512
local storage_accept = 141230
local storage_check = 143201
local guild_invite = 532351
local guild_accept = 446121
local time_1_pos = {x = 1022, y = 1064, z = 2}
local time_2_pos = {x = 1137, y = 1032, z = 2}
local city = "desert"
...
function onSay(cid, words, param, channel)
        ...
        if getGuildId(t[2]) and getGuildId(t[2]) ~= getPlayerGuildId(cid) then
            if #getOnlineGuildMembersByRank(getGuildId(t[2]), 3, false) > 0 then
                if getGlobalStorageValue(arena_storage) <= os.time() then
                    doPlayerSendTextMessage(cid, MESSAGE_STATUS_CONSOLE_ORANGE, "Voce convidou a guild "..t[2].." para uma war em "..city.." war!")
                    for _, gid in pairs(getOnlineGuildMembersByRank(getGuildId(t[2]), 3, false)) do
                        doPlayerSendTextMessage(gid, MESSAGE_STATUS_CONSOLE_ORANGE, getCreatureName(cid).." da guild "..getPlayerGuildName(cid).." chamou sua guild para uma war em "..city.." Para aceitar diga /desertwar accept, "..getPlayerGuildName(cid)..".")
                        setPlayerStorageValue(gid, storage_accept, os.time() + 20*60)
                        setPlayerStorageValue(gid, storage_check, getPlayerGuildId(cid))
                    end
                else
                    doPlayerSendTextMessage(cid, MESSAGE_STATUS_CONSOLE_BLUE, "Desert war ja esta ocupada.")
                end
            else
        ...
        if getGuildId(t[2]) and getGuildId(t[2]) ~= getPlayerGuildId(cid) then
            if getPlayerStorageValue(cid, storage_accept) > os.time() then
                if getPlayerStorageValue(cid, storage_check) == getGuildId(t[2]) then
                    setGlobalStorageValue(arena_storage, os.time() + 1800)
                    ...
                    if #getOnlineGuildMembersByRank(getGuildId(t[2]), 3, true) > 0 then
                        ...
                        doPlayerSendTextMessage(gid, MESSAGE_STATUS_CONSOLE_BLUE, "Sua guild entrou em war contra a guild "..getPlayerGuildName(cid).."! Para participar diga /desertwar go")
                        end
                    end
                    if #getOnlineGuildMembersByRank(getPlayerGuildId(cid), 3, true) > 0 then
                        for _, pid in pairs(getOnlineGuildMembersByRank(getPlayerGuildId(cid), 3, true)) do
                            doPlayerSendTextMessage(pid, MESSAGE_STATUS_CONSOLE_BLUE, "Sua guild entrou em war contra a guild "..t[2].."! Para participar diga /desertwar go")
                        end
                    end
                    addEvent(function()
                        for _, pid in pairs(getPlayersOnline()) do
                            if isInArea(getThingPos(pid), {x = 1151, y = 1085, z = 2}, {x = 1008, y = 1015, z = 2}) or isInArea(getThingPos(pid), {x = 1151, y = 1085, z = 2}, {x = 1008, y = 1015, z = 2}) or isInArea(getThingPos(pid), {x = 1151, y = 1085, z = 2}, {x = 1008, y = 1015, z = 2}) or isInArea(getThingPos(pid), {x = 1151, y = 1085, z = 2}, {x = 1008, y = 1015, z = 2}) or isInArea(getThingPos(pid), {x = 1151, y = 1085, z = 2}, {x = 1008, y = 1015, z = 2}) or isInArea(getThingPos(pid), {x = 1151, y = 1085, z = 2}, {x = 1008, y = 1015, z = 2}) or isInArea(getThingPos(pid), {x = 1151, y = 1085, z = 2}, {x = 1008, y = 1015, z = 2}) then
                                doTeleportThing(pid, getTownTemplePosition(getPlayerTown(cid)))
                                doPlayerSendTextMessage(pid, MESSAGE_STATUS_CONSOLE_BLUE, "Voce foi teleportado para o templo pq o tempo de war acabou")
                            end
                        end
                    end, 1000*1800)
                    addEvent(setGlobalStorageValue, 1800*1000, guild_accept, -1)
                    addEvent(setGlobalStorageValue, 1800*1000, guild_invite, -1)
                else
    ...
    elseif(t[1] == "go") then
        ...
        if getGlobalStorageValue(guild_invite) == getPlayerGuildId(cid) or getGlobalStorageValue(guild_accept) == getPlayerGuildId(cid) then
            ...
            doPlayerSendCancel(cid, "Sua guild nao esta em desert war")
        end
    end
    return true
end
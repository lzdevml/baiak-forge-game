	...
	if(k[1] ~= nil) then
		if(string.upper(k[1]) == 'INFO') then
			if(getPlayerStorageValue(cid, SBW_INEVENT) == 1) and (not getTileInfo(getThingPos(cid)).protection) then
				...
				doPlayerSendTextMessage(cid, 27, "[SnowBall WAR] Para utilizar este comando, você precisa estar dentro do evento.") 
				...
				if(getPlayerStorageValue(cid, SBW_AMMO) > 0) then
					if SBW_STATUS == 'on' then
						...
						if (getPlayerStorageValue(cid, SBW_AMMO) ~= 1) then
							doPlayerSendTextMessage(cid, 27, "[SnowBall WAR] Restam "..getPlayerStorageValue(cid, SBW_AMMO).."x bolas de neve.")
						else
							doPlayerSendTextMessage(cid, 27, "[SnowBall WAR] Usando sua última bola de neve.")
						end
					end
				else
					doPlayerSendCancel(cid, "[SnowBall WAR] Você está sem bolas de neve! Recarregue sua munição no centro do campo.")
					doSendMagicEffect(getCreaturePosition(cid), CONST_ME_POFF)     
				end
			else
				doPlayerSendTextMessage(cid, 27, "[SnowBall WAR] Para utilizar este comando, você precisa estar dentro do evento.") 
...
function killPlayer(cid, pos, killer)
	...
	doSendAnimatedText(pos, "-1 ", COLOR_RED)
	doSendAnimatedText(getCreaturePosition(killer), "+1 ", COLOR_GREEN)
	doPlayerSendTextMessage(cid, 27, "[SnowBall WAR] Você foi morto por "..getCreatureName(killer)..".")
	doPlayerSetStorageValue(killer, SBW_SCORE, getPlayerStorageValue(killer, SBW_SCORE)+SBW_POINTSKILL)				
	doPlayerSendTextMessage(killer, 27, "[SnowBall WAR] Você matou "..getCreatureName(cid)..".")

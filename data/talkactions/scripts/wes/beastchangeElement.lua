...
local config = {
	...
	msg = {
		help = "Use o comando exemplo: !beast holy. Efeitos disponíveis: ice, fire, earth, energy, holy, death, e none para desabilitar. A troca para outro elemento custa 10k.",
		invalid = "Por favor, digite um tipo valido.",
		changed = "O elemento da beast foi trocado para %s.",
		nomoney = "Você precisa de 10.000 gold coins para utilizar o comando!",
		delayed = "Você precisa esperar para trocar o efeito.",
	}
}

-- /////////////////////Requerimentos///////////////////--
local req_itemid = 12835                                  -- ID do item requerido
...
if getPlayerItemCount(cid, req_itemid) > 0 then
	if param == "help" or param == "" then
		doShowTextDialog(cid, 12835, string.format(config.msg.help, config.price))
		return true
	end
